/**
 * NaFome — Gerador de Páginas Individuais por Receita
 *
 * Como usar:
 *   node gerar-receitas.js
 *
 * Lê os JSONs em data/receitas/ e gera uma página HTML
 * por receita em receitas/[slug]/index.html
 */
...
function gerarHTML(receita, nichoSlug) {
  const titulo    = receita.titulo || 'Receita';
  const slug      = receita.slug  || '';
  const descricao = receita.descricao || '';
  const canonical = `${BASE_URL}/receitas/${slug}/`;
  const sorteioURL = `${BASE_URL}/?nicho=${nichoSlug}`;
  const nichoURL   = `${BASE_URL}/pages/${nichoSlug}/`;

  const metaDesc = `Como fazer ${titulo}: ingredientes, modo de preparo passo a passo${receita.tempo ? ', pronto em ' + receita.tempo : ''}${receita.preco ? ', por apenas ' + receita.preco : ''}. Receita fácil no NaFome.`;

  const ingredientesHTML = Array.isArray(receita.ingredientes)
    ? receita.ingredientes.map(i => `<li>${i}</li>`).join('\n            ')
    : '';

  const passosHTML = Array.isArray(receita.modoPreparo)
    ? receita.modoPreparo.map((p, idx) => `<li><span class="step-num">${idx + 1}</span>${p}</li>`).join('\n            ')
    : '';
  ...
  const schemaReceita = {
    ...
    "url": canonical,
    "recipeIngredient": Array.isArray(receita.ingredientes) ? receita.ingredientes : [],
    "recipeInstructions": Array.isArray(receita.modoPreparo)
      ? receita.modoPreparo.map((p, i) => ({
          "@type": "HowToStep",
          "position": i + 1,
          "text": p
        }))
      : [],
    ...(receita.tempo       ? { "totalTime": receita.tempo } : {}),
    ...(receita.porcoes     ? { "recipeYield": receita.porcoes } : {}),
    "isPartOf": { "@type": "WebSite", "name": "NaFome", "url": BASE_URL }
  };
...
<head>
  ...
  <title>${titulo} — Receita Fácil | NaFome</title>
  ...
  ${JSON.stringify(schemaReceita, null, 2)}
  <\/script>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-sortear:hover { background: #e55a25; transform: scale(1.03); text-decoration: none; }
    ...
    @media (max-width: 600px) {
      .receita-body { grid-template-columns: 1fr; }
    }
    ...
    .preparo li:last-child { border-bottom: none; }
    .step-num {
      background: #ff6b35;
      color: #fff;
      width: 24px;
    ...
    .cta-box h2 { font-size: 20px; font-weight: 800; color: #fff; margin-bottom: 8px; }
    ...
    .btn-cta:hover { transform: scale(1.04); text-decoration: none; color: #e55a25; }
    .btn-cta.secondary {
...
<body>
  ...
  <nav class="topbar">
    ...
    <a href="${sorteioURL}" class="btn-sortear">🎲 Sortear outra receita</a>
  </nav>

  <div class="breadcrumb">
    <a href="${BASE_URL}">NaFome</a>
    <span>›</span>
    <a href="${nichoURL}">${slugParaCategoria(receita.categoria)}</a>
    <span>›</span>
    ${titulo}
  </div>
  ...
  <div class="container">
    ...
    <div class="receita-header">
      <h1 class="receita-titulo">${titulo}</h1>
      <p class="receita-desc">${descricao}</p>
      <div class="badges">
        ${receita.tempo       ? `<span class="badge">⏱ ${receita.tempo}</span>` : ''}
        ${receita.dificuldade ? `<span class="badge">📊 ${receita.dificuldade}</span>` : ''}
        ${receita.preco       ? `<span class="badge">💰 ${receita.preco}</span>` : ''}
        ${receita.porcoes     ? `<span class="badge">👤 ${receita.porcoes}</span>` : ''}
      </div>
    </div>
    ...
    <div class="cta-box">
      ...
      <a href="${sorteioURL}" class="btn-cta">🎲 Sortear desta categoria</a>
      <a href="${BASE_URL}" class="btn-cta secondary">🍳 Ver todos os nichos</a>
    </div>
...
function main() {
  console.log('🍳 NaFome — Gerador de Páginas Individuais por Receita\n');
  ...
  arquivos.forEach(arquivo => {
    const nichoSlug = arquivo.replace('.json', '');
    ...
    try {
      ...
      console.warn(`⚠️  ${arquivo} — erro ao ler: ${e.message}`);
      return;
    }

    receitas.forEach(receita => {
      if (!receita.slug) {
        console.warn(`⚠️  Receita sem slug em ${arquivo}: ${receita.titulo}`);
        return;
      }

      const pasta = path.join(OUTPUT_DIR, receita.slug);
      if (!fs.existsSync(pasta)) {
        fs.mkdirSync(pasta, { recursive: true });
      }

      const html = gerarHTML(receita, nichoSlug);
      fs.writeFileSync(path.join(pasta, 'index.html'), html, 'utf8');
      totalPaginas++;
    });

    console.log(`✅ ${nichoSlug} — ${receitas.length} receitas geradas`);
  });

  console.log(`\n🎉 Concluído!`);
  console.log(`   ${totalPaginas} páginas individuais geradas`);
